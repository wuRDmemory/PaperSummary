...
这里笔者打算先进行第三章，先了解一下系统的实际设计，再回来看第二章的理论部分。然后后面的章节就不做赘述了。
...
- 把距离分数最大的帧边缘化掉，距离分数公式为：
  ...
  可以看到该公式就是在找最**离群**的帧，保证剩下的帧都和最新的帧距离很近；
...
相应的，在计算Jacobian时要把$x$设置为0，也就是要用第一次边缘化时候的线性化点$\zeta_0$，以此来保证系统的可观性不被改变；例如在k时刻进行了第一次边缘化，边缘化了$U$帧，关联帧为$\Lambda$，无关帧为$V$，记此时的估计量为整个系统的状态量为$\zeta_0$，那么在之后的优化过程中，$\Lambda$的线性化点是不能变的，要保持k时刻的参数$\zeta_{0\Lambda}$（即Jacobian保持相同，亦即迭代的方向要保持一致，看下图），而通过增量方程计算出来的增量用来更新除了被边缘化之外的参数。

> 这里记录一个小问题，以后读完代码或者相应的文章再回来补充：
>
> 1. 如上例子，因为$\Lambda$与$U$是关联的，因此使用固定线性化点没毛病，那对于$V$呢？线性化点要固定吗？
> 2. FEJ保持的线性化点全程保持不变吗？如果是，那么当第一次边缘化的时候所有的关联帧$\Lambda$都被边缘化之后，是不是FEJ的作用就没有了？如果不是，这个的线性化点又是如何更新的？
...
就整个总结而言，其中一定不乏一些不对的地方，还请得道的大佬们指出来，笔者会即使改进，提前感激不尽！
